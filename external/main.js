...
function createPopularBlock(blockData) {
    const point = blockData.places.length === 0
        ? '<span>Москва, Турция</span>'
        : blockData.places.map(place => `<span>${place}</span>`).join(', ');
    return `
        <a href="${blockData.link}" target="_blank" class="popular-block">
...
function createPopularWrapper(popularData) {
    ...
    popularData.forEach(block => {
        popularContainer.insertAdjacentHTML('beforeend', createPopularBlock(block));
    });
