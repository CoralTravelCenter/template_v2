(() => {
    ...
    const suits = document.querySelector('[data-node-key="Номера"]');

    console.log(suits);

    suits.addEventListener('click', (e) => {
        const obs = new MutationObserver(() => {
            const allAntTagSpans = document.querySelectorAll('span.ant-tag');

            const targetSpans = Array.from(allAntTagSpans).filter(span => {
                if (!span.textContent.includes('Особые привилегии')) {
                    return false;
                }

                const nextEliteDiamond = span.nextElementSibling?.classList?.contains('elite-diamond');

                return !nextEliteDiamond;
            });
