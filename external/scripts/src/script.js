...
// Убедимся, что dataLayer существует
        ...
        for (const item of args) {
            ...
            for (const tour of items) {
                ...
                mindbox('async', {
                    ...
                    data: {
                        customerAction: {
                            customFields: {
                                ...
                                flightDateFrom,
                                flightDateTo,
                                // если в логе есть даты отеля — замените эти строки
                    ...
                    onSuccess: () => console.log('✅ SearchTour sent to Mindbox'),
                    onError:   err => console.error('❌ SearchTour error', err)
                });
            }
        }

        // вызываем оригинальный push, чтобы не сломать другие слушатели
