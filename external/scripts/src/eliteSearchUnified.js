(function () {
    ...
    const EliteState = (() => {
        ...
        return {
            ...
            shouldAutoOpenPopup() {
                if (this.isEliteDismissedPermanently()) return false;
    ...
    const ElitePopup = ((state, analytics) => {
        const COUNTRY_LINKS = {
        ...
        function ensureStyles() {
            if (document.querySelector('style[data-elite-popup-style]')) return;
    ...
    `;
        ...
        function open({isAutoOpen = false} = {}) {
      ...
      <div class="elite-popup" data-elite-popup>
        <div class="elite-popup__wrapper" role="dialog" aria-modal="true">
          ...
          <div class="elite-popup__info">
            ...
            <a href="#" class="elite-popup__link" data-elite-popup-link aria-disabled="true">Посмотреть подборку</a>
            <a href="#" class="elite-popup__back" data-elite-popup-back>Нет, спасибо</a>
          </div>
        </div>
      </div>
    `);
            ...
            if (isAutoOpen) {
                analytics.reachGoal('personalization_elite_pop_up_show');
            }
            ...
            const closePopupAndShowShield = () => {
                analytics.reachGoal('personalization_elite_pop_up_click', {
                    button: 'close',
            ...
            const setSelected = (country) => {
                ...
                items.forEach((el) => {
                    el.setAttribute('aria-selected', String(el.getAttribute('data-value') === country));
                });
            ...
            const openDd = () => {
                ...
                panel.hidden = false;
                activeIndex = items.findIndex((item) => item.getAttribute('data-value') === selectedValue);
            ...
            link.addEventListener('click', (e) => {
                const countryForMetric = selectedValue || 'not selected';
            ...
            backBtn?.addEventListener('click', (e) => {
                ...
                analytics.reachGoal('personalization_elite_pop_up_click', {
                    button: 'no',
                    country: selectedValue || 'not selected',
                });
    ...
    })(EliteState, Analytics);

    const EliteShield = ((state, analytics, popup) => {
    ...
    })(EliteState, Analytics, ElitePopup);

    const EliteSearchInterceptor = ((state) => {
        ...
        function activateEliteChainFromUrl() {
            let url;
        ...
        function clearEliteChainIfOutsideSearch(url = window.location.href) {
            if (!isSearchPage(url)) {
                state.clearEliteSearch();
            }
        }

        function syncEliteChainState(url = window.location.href) {
            if (isSearchPage(url)) {
    ...
    const EliteApp = ((state, analytics, popup, shield, interceptor) => {
        return {
            async init() {
    ...
    })(EliteState, Analytics, ElitePopup, EliteShield, EliteSearchInterceptor);
